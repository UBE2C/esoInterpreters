...
Matrix initMatrix(DataType type, uint32_t n_row, uint32_t n_col) {
    ...
    switch (type) {
        ...
        case STR: data_size = sizeof(char*); mat.data.str = calloc(n_row * n_col, data_size); break; //for a string matrix n_col must be equal to longer than the null terminated string length
        case EMPTY:
...
Status setMat_u32(uint32_t value_in, Matrix* matrix, uint32_t row, uint32_t col) {
    if (matrix == NULL) {
        printf("setMat_u32: error, NULL pointer provided.\n");
        return STAT_ERROR;

    }

    if (matrix -> data.u32p == NULL) {
        printf("setMat_u32: error, the data pointer is NULL.\n");
        return STAT_ERROR;
    ...
    uint32_t index = row * matrix -> n_col + col;
    if (index >= matrix -> n_row * matrix -> n_col) {
        printf("setMat_u32: error, index outside matrix bounds.\n");
        return STAT_ERROR;

    }

    matrix -> data.u32p[index] = value_in;
    return STAT_SUCCESS;

}

Status setMat_u64(uint64_t value_in, Matrix* matrix, uint32_t row, uint32_t col) {
    if (matrix == NULL) {
        printf("setMat_u64: error, NULL pointer provided.\n");
        return STAT_ERROR;

    }

    if (matrix -> data.u64p == NULL) {
        printf("setMat_u64: error, the data pointer is NULL.\n");
        return STAT_ERROR;
    ...
    uint32_t index = row * matrix -> n_col + col;
    if (index >= matrix -> n_row * matrix -> n_col) {
        printf("setMat_u64: error, index outside matrix bounds.\n");
        return STAT_ERROR;

    }

    matrix -> data.u64p[index] = value_in;
    return STAT_SUCCESS;

}

Status setMat_i8(int8_t value_in, Matrix* matrix, uint32_t row, uint32_t col) {
    if (matrix == NULL) {
        printf("setMat_i8: error, NULL pointer provided.\n");
        return STAT_ERROR;

    }

    if (matrix -> data.i8p == NULL) {
        printf("setMat_i8: error, the data pointer is NULL.\n");
        return STAT_ERROR;
    ...
    uint32_t index = row * matrix -> n_col + col;
    if (index >= matrix -> n_row * matrix -> n_col) {
        printf("setMat_i8: error, index outside matrix bounds.\n");
        return STAT_ERROR;

    }

    matrix -> data.i8p[index] = value_in;
    return STAT_SUCCESS;

}

Status setMat_i32(int32_t value_in, Matrix* matrix, uint32_t row, uint32_t col) {
    if (matrix == NULL) {
        printf("setMat_i32: error, NULL pointer provided.\n");
        return STAT_ERROR;

    }

    if (matrix -> data.i32p == NULL) {
        printf("setMat_i32: error, the data pointer is NULL.\n");
        return STAT_ERROR;
    ...
    uint32_t index = row * matrix -> n_col + col;
    if (index >= matrix -> n_row * matrix -> n_col) {
        printf("setMat_i32: error, index outside matrix bounds.\n");
        return STAT_ERROR;

    }

    matrix -> data.i32p[index] = value_in;
    return STAT_SUCCESS;

}

Status setMat_i64(int64_t value_in, Matrix* matrix, uint32_t row, uint32_t col) {
    if (matrix == NULL) {
        printf("setMat_i64: error, NULL pointer provided.\n");
        return STAT_ERROR;

    }

    if (matrix -> data.i64p == NULL) {
        printf("setMat_i64: error, the data pointer is NULL.\n");
        return STAT_ERROR;
    ...
    uint32_t index = row * matrix -> n_col + col;
    if (index >= matrix -> n_row * matrix -> n_col) {
        printf("setMat_i64: error, index outside matrix bounds.\n");
        return STAT_ERROR;

    }

    matrix -> data.i64p[index] = value_in;
    return STAT_SUCCESS;

}

Status setMat_f32(float value_in, Matrix* matrix, uint32_t row, uint32_t col) {
    if (matrix == NULL) {
        printf("setMat_f32: error, NULL pointer provided.\n");
        return STAT_ERROR;

    }

    if (matrix -> data.f32p == NULL) {
        printf("setMat_f32: error, the data pointer is NULL.\n");
        return STAT_ERROR;
    ...
    uint32_t index = row * matrix -> n_col + col;
    if (index >= matrix -> n_row * matrix -> n_col) {
        printf("setMat_f32: error, index outside matrix bounds.\n");
        return STAT_ERROR;

    }

    matrix -> data.f32p[index] = value_in;
    return STAT_SUCCESS;

}

Status setMat_f64(double value_in, Matrix* matrix, uint32_t row, uint32_t col) {
    if (matrix == NULL) {
        printf("setMat_f64: error, NULL pointer provided.\n");
        return STAT_ERROR;

    }

    if (matrix -> data.f64p == NULL) {
        printf("setMat_f64: error, the data pointer is NULL.\n");
        return STAT_ERROR;
    ...
    uint32_t index = row * matrix -> n_col + col;
    if (index >= matrix -> n_row * matrix -> n_col) {
        printf("setMat_f64: error, index outside matrix bounds.\n");
        return STAT_ERROR;

    }

    matrix -> data.f64p[index] = value_in;
    return STAT_SUCCESS;

}

Status setMat_str(char* value_in, Matrix* matrix, uint32_t row, uint32_t col) {
    if (matrix == NULL) {
        printf("setMat_str: error, NULL pointer provided.\n");
        return STAT_ERROR;

    }

    if (matrix -> data.str == NULL) {
        printf("setMat_str: error, the data pointer is NULL.\n");
        return STAT_ERROR;
    ...
    uint32_t index = row * matrix -> n_col + col;
    if (index >= matrix -> n_row * matrix -> n_col) {
        printf("setMat_str: error, index outside matrix bounds.\n");
        return STAT_ERROR;

    }

    char* cpyRet = strncpy(matrix -> data.str[index], value_in, matrix -> n_col);
    if (cpyRet == NULL) {
        printf("setMat_str: error, string copy failed.\n");
        return STAT_ERROR;
    }
